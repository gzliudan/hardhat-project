...
describe('MhToken contract', function () {
  async function deployTokenFixture() {
    ...
    const hardhatToken = await ethers.deployContract('MhToken');
  ...
  describe('Transactions', function () {
    ...
    it("Should fail if sender doesn't have enough tokens", async function () {
      ...
      await expect(hardhatToken.connect(addr1).transfer(owner.address, 1)).to.be.revertedWithCustomError(hardhatToken, `InsufficientToken`);
